 ...
 * Last modified by suhler on 09/06/03 10:52:47
 *
 * Version Histories:
 *
 * 2.8 09/06/03-10:52:47 (suhler)
 *   changed some variable scoping to make it easier for subclasses
 *   to modify behavior
 *
...
public class LexML
{
	...
	if (s == null) {
	    ...
	    this.strEnd = s.length();
	    done=(tokenEnd >= strEnd);
	}
    }

    private int
    skip(String pattern, String s, int i, int end)
    {
	for ( ; i < end; i++) {
	    if (pattern.indexOf(s.charAt(i)) < 0) {
		break;
	    }
	}
	return i;
    ...
    private int
    next(String pattern, String s, int i, int end)
    {
	for ( ; i < end; i++) {
	    if (pattern.indexOf(s.charAt(i)) >= 0) {
		break;
