 ...
 * Last modified by suhler on 09/10/12 12:51:05
 *
 * Version Histories:
 *
 * 2.11 09/10/12-12:51:05 (suhler)
 *   add "allwaysCancel" config option
 *
 * 2.10 09/09/16-16:31:36 (suhler)
 *   oops
 *
 * 2.9 09/09/16-16:11:01 (suhler)
 *   allow file suffixes to work with the "default" file (e.g the
 *   url ends with /).  This avoids needing a DefaultFileHandler in
 *   simple cases
 *
 ...
 * <dd>
 * <dt> <code>alwaysCancel</code>
 * <dd> If true, this handler always returns false, allowing
 * further request processing to take place.  This allows templates
 * to be run entirely for their side-effects.  See also the
 * {@link  sunlabs.brazil.sunlabs.CancelTemplate}, which allows this
 * behavior to be implemented selectively in the template markup.
 * 
 * </dl>
...
public class TemplateHandler implements Handler {
    ...
    String propsPrefix;		// our prefix in the config file
    protected String sessionProperty;	// where to find the session name
    protected MatchString isMine;         // check for matching url
    protected boolean modified;		// if set, emit last-modified header
    String encoding;		// the page character encoding

    Handler handler;
    protected TemplateRunner runner;		// The template object for our class
	...
	String str;
	str = server.getProps().getProperty(propsPrefix + TEMPLATES);
	if (str == null) {
	...
	boolean alwaysCancel = Format.isTrue(props.getProperty(propsPrefix +
		"alwaysCancel"));

	String url = request.getUrl();

	// We could do url mapping here if we wanted to, but
	// for now we'll let the UrlMapperHandler do it for us.
	// url = props.getProperty(propsPrefix + "url", request.url);

	// if we are using suffixes, make sure the "default" file works

	String indexFile = props.getProperty(propsPrefix+DEFAULT, "index.html");
	if (!isMine.match(url.endsWith("/") ? url + indexFile : url)) {
	    return false;
      }

	String root = props.getProperty(propsPrefix + FileHandler.ROOT,
		props.getProperty(FileHandler.ROOT, "."));
	String name = FileHandler.urlToPath(url);
...
*/
	...
	if (result != null && !alwaysCancel) {
	    if (modified) {
	       request.addHeader("Last-Modified", HttpUtil.formatTime());
	    }
	    // XXX This doesn't work on appengine either
	    // FileHandler.setModified(request.getProps(),file.lastModified());
	    ...
	    return true;
	} else if (alwaysCancel) {
	    request.log(Server.LOG_INFORMATIONAL,propsPrefix, "alwaysCancel");
	} else {
