 ...
 * Last modified by suhler on 09/07/08 07:52:32
 *
 * Version Histories:
 *
 * 2.19 09/07/08-07:52:32 (suhler)
 *   doc updates
 *
 * 2.18 09/07/07-14:50:25 (suhler)
 *   Make the "load()" and "store()" methods public so other classes
 *   can use them
 *
 * 2.17 09/02/12-14:06:42 (suhler)
 *   Document "track" attribute, change tracking from "stdout" to "stderr"
 *
 ...
 *     if they match the glob pattern. [Deprecated, use
 *     queryPrefix instead]
 * <dt>queryPrefix
 * <dd>If set, all query parameters will be placed in the 
 *     request properties prefixed by "queryPrefix".
 *     This is the same as "query" for the template version;
 *     The names are different for historical reasons.
 * <dt>autoImport
...
public class SetTemplate extends Template implements Serializable, Handler {
  ...
  String queryPrefix;		// put query parameters into request props
  ...
  Vector<String> remember = new Vector<String>();	// future chainable namespaces
  ...
  init(RewriteContext hr) {
    ...
    if (query != null) {
      Dictionary<String, String> h = hr.request.getQueryData(null);
    ...
    if (headers != null) {
      Enumeration<String> keys = hr.request.getHeaders().keys();
      while(keys.hasMoreElements()) {
        String key = keys.nextElement();
        props.put(headers + key.toLowerCase(),
            hr.request.getHeaders().get(key));
      }
      ...
      String protocol = hr.request.getServerProtocol() == null ?
          ...
          props.put(headers + "address", "unknown"); // XXX fix
          props.put(headers + "timestamp", "" + hr.request.getStartMillis());
    ...
    mustMatch = hr.request.getProps().getProperty(hr.prefix + "mustMatch");
    allowServer = (null == hr.request.getProps().getProperty(hr.prefix +
    "noserver"));
    noSet = (null != hr.request.getProps().getProperty(hr.prefix +
    "noSet"));
    sessionTable = hr.request.getProps().getProperty(hr.prefix +
        "sessionTable", hr.templatePrefix);

    String autoImport = hr.request.getProps().getProperty(hr.prefix +
        "autoImport", "1");
    ...
    String imports = hr.request.getProps().getProperty(hr.prefix + "imports");
    if (imports != null) {
      StringTokenizer st = new StringTokenizer(imports);
      while (st.hasMoreTokens()) {
        String token = Format.subst(hr.request.getProps(), st.nextToken());
   ...
   *     request.props.
   * <dt>track
   * <dd>If true, all "set" operations are logged to stderr.  This
   *     paramter may also be set in the configuration file to effect
   *     all "set" tags.
   * </dl>
   */
  ...
  tag_set(RewriteContext hr) {
    boolean shouldTrack = Format.isTrue(hr.request.getProps().getProperty(hr.prefix + "track"));
    if (noSet) {
      return;
    }
    debug(hr);
    ...
    Class<? extends Properties> create  = (value != null)  ? Properties.class : null;
    ...
    if (value != null) {
      ...
      p.remove(name);
      if (shouldTrack) {
        System.err.println("<set> Clearing the entry (" + name +
  ...
  tag_get(RewriteContext hr) {
    ...
    if (name.indexOf('=') >= 0) {
      ...
      name = Format.subst(hr.request.getProps(), name);
    }
    ...
    if (namespace != null) {
      ...
      result = hr.request.getProps().getProperty(name, result);
    }
    ...
    if (result != null) {
      ...
      if (set != null) {
        hr.request.getProps().put(set, result);
      } else {
        hr.append(result);
      }
    } else {
   ...
   * <p>
   * If "prefix" is specified (and the namespace already exists),
   * the the properties of this namespace will only be enumerated
   * with &lt;foreach ... glob=foo..."&gt; if the glob pattern ("foo") begins
   * with the prefix. Thus only entries in the namspace begginging
   * with the "prefix" value can be enumerated, although all the
   * values can be retrieved using &lt;get..."&gt; or ${...}.
   */
  ...
  tag_import(RewriteContext hr) {
    ...
    if (namespace != null) {
      Properties p = (Properties) SessionManager.getSession(namespace,
          sessionTable, null);
      if (p != null) {
        hr.request.addSharedProps(p, hr.get("prefix"));
  ...
  tag_unimport(RewriteContext hr) {
    ...
    if (namespace != null) {
      Properties p = (Properties) SessionManager.getSession(namespace,
          sessionTable, null);
      if (p != null) {
        PropertiesList pl = hr.request.getProps().wraps(p);
        if (pl != null) {
          pl.remove();
          hr.request.log(Server.LOG_DIAGNOSTIC, hr.prefix,
  ...
  tag_namespace(RewriteContext hr) {
    ...
    if (name != null) {
      if (!hr.isSingleton()) {
        current = name;
        hr.request.getProps().put(hr.templatePrefix + "namespace", current);
      }
      
      // Alternative to load/store?
      if (hr.isTrue("persist")) {
        Properties p = (Properties) SessionManager.getSession(name,
            sessionTable, null);
        if (p != null) {
          SessionManager.put(name, sessionTable, p);
      ...
      boolean ok =
        (null != hr.request.getProps().getProperty(hr.prefix + "saveOk"));
  ...
  /*
   * XXX
   * load() and store() should be redone as an interface/factory
   * to allow different implementations (e.g. using a database instead
   * of the filesystem) to be installed at deployment time.
   */
   ...
   * load a namespace from a java properties file
	...
	File file = file2path(hr.request.getProps(), hr.prefix, name);
	hr.request.log(Server.LOG_DIAGNOSTIC, hr.prefix,
   ...
   *XXX This needs to be replaced by an interface
   */
  /*
    public static boolean
	...
	try {
	    ...
	    p.save(out, hr.request.serverUrl() + hr.request.getUrl() +
		" (for namespace " + title + ")");
	    out.close();
	    hr. request.log(Server.LOG_DIAGNOSTIC, hr.prefix,
  ...
  public static boolean
  store(Properties p, RewriteContext hr, String name, String title) {
    SessionManager.put("set", name, p);
    return true;
  }
  
  public static boolean
  load(Properties p, RewriteContext hr, String name) {
    Properties fetch = (Properties) SessionManager.get("set", name);
    System.err.println("name=" + fetch);
    if (fetch != null) {
      Enumeration e = fetch.keys();
      while(e.hasMoreElements()) {
        String key = (String) e.nextElement();
        p.put(key, fetch.get(key));
      }
    }
  ...
  tag_slash_namespace(RewriteContext hr) {
    ...
    hr.killToken();
    current = null;
    hr.request.getProps().remove(hr.templatePrefix + "namespace");
  }
  ...
  init(Server server, String prefix) {
    this.prefix = prefix;
    querySet = server.getProps().getProperty(prefix + "querySet");
    queryPrefix = server.getProps().getProperty(prefix + "queryPrefix");
    sessionTable = server.getProps().getProperty(prefix + "sessionTable",
        prefix);
    session=server.getProps().getProperty(prefix + "session", "SessionID");
  ...
  respond(Request request) {
    String id = request.getProps().getProperty(session);
    if (id != null) {
    ...
    if (querySet!=null) {
      Dictionary<String, String> h = request.getQueryData(null);
      Enumeration<String> keys = h.keys();
      while(keys.hasMoreElements()) {
        String key = keys.nextElement();
        if (Glob.match(querySet, key)) {
          request.getProps().put(key, h.get(key));
        }
      }
    }
    ...
    if (queryPrefix!=null) {
      ...
      while(keys.hasMoreElements()) {
        String key = keys.nextElement();
        request.getProps().put(queryPrefix + key, h.get(key));
      }
    }

    /*
     * Put query parameters into request props with prefix
     */
