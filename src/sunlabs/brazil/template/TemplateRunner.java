 ...
 * Last modified by suhler on 08/03/04 10:56:59
 *
 * Version Histories:
 *
 * 2.5 08/03/04-10:56:59 (suhler)
 *   javadoc fixes
 *
...
public class TemplateRunner
{
    ...
    private Class<? extends TemplateInterface>[] types;		// Types of templates.
  ...
  public boolean filter(Regsub rs, StringBuffer sb) {
	...
	while (st.hasMoreTokens()) {
	    ...
	    String className = server.getProps().getProperty(temName + ".class");
	    if (className == null) {
		className = temName;
		temPrefix = prefix;
	    } else {
	    ...
	    String tagPrefix = server.getProps().getProperty(temPrefix + TAG_PREFIX,
		    server.getProps().getProperty(prefix + TAG_PREFIX, ""));
