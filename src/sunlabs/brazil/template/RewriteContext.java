 ...
 * Last modified by suhler on 09/09/24 10:36:18
 *
 * Version Histories:
 *
 * 2.11 09/09/24-10:36:18 (suhler)
 *   remove "snarfed" markup from emitted code
 *
 * 2.10 09/06/04-11:24:01 (suhler)
 *   lint
 *
...
public class RewriteContext extends HtmlRewriter {
    ...
    get(String name, boolean subst) {
	  if (subst) {
	    return Format.subst(request.getProps(), super.get(name));
	  } else {
	...
	while(nextToken()) {
		...
		nestingLevel1++;
	    } else if (tag.equals("/" + match) && (--nestingLevel1 < 0)) {
		nextToken();
		break;
	    }
	    sb1.append(getToken());
    ...
    String getTagPrefix() {
	return server.getProps().getProperty(prefix + TemplateRunner.TAG_PREFIX,
		    server.getProps().getProperty(templatePrefix +
     ...
     * look at any attributes of the current tag first,
     * then look in Supplied Properties.
     *
     * Example usage:
     * <pre>
     *	 Format.subst(new AttributePropsProps(rewriteContext), data);
     * </pre>
     */

    public static class AttributeProps extends Properties {
        RewriteContext hr;
	Properties props;

	public AttributeProps(RewriteContext hr) {
	    this(hr, hr.request.getProps());
	}

	public AttributeProps(RewriteContext hr, Properties props) {
	    this.hr=hr;
	    this.props = props;
	}
