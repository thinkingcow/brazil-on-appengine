 ...
 * Last modified by suhler on 09/09/30 16:26:59
 *
 * Version Histories:
 *
 * 2.22 09/09/30-16:26:59 (suhler)
 *   doc updates (mostly)
 *
 * 2.21 09/06/30-09:34:58 (suhler)
 *   add "index=nnn" to <foreach> to provide the iteration count of each item,
 *   starting at nnn.
 *
 * 2.20 09/06/03-10:55:58 (suhler)
 *   added "track" option to track some variables on the console
 *
 * 2.19 09/01/20-16:13:41 (suhler)
 *   use static inner class
 *
 * 2.18 08/08/29-10:04:05 (suhler)
 *   doc fixes
 *
 * 2.17 08/07/24-16:35:12 (suhler)
 ...
 * <dt> <code>&lt;if expr="<var>numeric expression</var>" [value=...]&gt;</code>
 ...
 *	Any vaiable that is defined, but not "0", "off", or "no" is considered
 ...
 *      <p>
 *	If "value" is specified along with "expr", then the result
 *	of the expression must exactly match the the supplied value.
 * </dl>
 * Normally, the variable is look-up in the Properties Stack, starting
 * with the Request Properties.  The attribute "namespace" may be used
 ...
 * <ul>
 * <li><pre>
 * &lt;foreach name=<var>var</var> list="<var>value1 value2 ...</var>" [delim="<var>chars</var> index=<var>i</var>"]&gt;
 *     &lt;get <var>var</var>&gt;
 *     <var>body</var>
 * &lt;/foreach&gt;</pre>
 *
 ...
 * <li>
 * If the optional parameter <code>index</code> is supplied, then at each
 * iteration, the value "name.count" counts the items, starting with the
 * supplied index value (or 1).
 * </ul>.
 *
 * <p><li><pre>
 * &lt;foreach name=<var>var</var> property=<var>property</var> [delim="<var>chars</var>" index="<var>i</var>"]&gt;
 *     &lt;get <var>var</var>&gt;
 *     <var>body</var>
 * &lt;/foreach&gt; </pre>
 *
 ...
 *
 * <p><li><pre>
 * &lt;foreach name=<var>var</var> glob=<var>pattern</var> [index=<var>i</var>]&gt;
 ...
 *
 * <p><li><pre>
 * &lt;foreach name=<var>var</var> match=<var>pattern</var> [index=<var>i</var>]&gt;
 *     &lt;get <var>var</var>.name&gt;
 *     &lt;get <var>var</var>.value&gt;
 *
 ...
 * <br>
 * If the optional parameter <code>index</code> is supplied, then
 * <code><var>var</var>.count</code>
 ...
 * insensitive match is performed.
 * <p>
 * When either <code>glob</code> or <code>match</code> is specified, then
 ...
 * specified namespace.
 * <p>BUG:
 * You must only specify a "namespace" if it has already been imported.
...
public class BSLTemplate extends Template implements Serializable {
	...
	if ((arg = hr.get("list")) != null) {
	    ...
	    arg = hr.request.getProps().getProperty(arg, "");
	    done = foreach_list(hr, name, arg, hr.get("delim"));
	}
	...
	if (done == false) {
	     ...
	     * was not processed, so we are not sitting at the </foreach>
	     foreach_match* and we need to skip to it.
  ...
  public int
	...
	int counter = Format.stringToInt(hr.get("index"), 1);
	boolean shouldCount = hr.get("index")!=null;
	...
	if ((sort == null) && (reverse == false)) {
		...
		if (shouldCount) local.push(name + ".count", "" + (counter++));
		any = true;
	...
	boolean was = hr.accumulate(true);
	hr.accumulate(was);
	boolean test = true;
	while (test) {
	    debug(hr);
	...
	if (name==null && expr != null) {
	    ...
	    try {
		String s = calc.getValue(expr);
		result = (value==null) ? Format.isTrue(s) : s.equals(value);
	...
	if ((arg = hr.get("value")) != null) {
	    ...
	    if (hr.isTrue("nocase")) {
     	  result = Glob.match(arg.toLowerCase(), value.toLowerCase());
	    } else  {
     	  result = Glob.match(arg, value);
	    }
	} else if (hr.isTrue("any")) {
	    result=hr.request.getProps().propertyNames(name).hasMoreElements();
	} else {
	    /*
	...
	String mapTable[] = {};
	boolean shouldTrack = Format.isTrue(hr.get("track", null));
	...
	String value = Format.getProperty(hr.request.getProps(), name, "");
	...
	if (glob != null) {
		...
		for (int i = 0; sub[i] != null; i++) {
		    ...
		    if (shouldTrack) {
				System.err.println("<extract>: " + prefix + s + "=" + sub[i]);
		    }
		    matches++;
		}
	    }
		...
		if (replace != null) {
		   ...
		   if (all) {	  // all matches
			   ...
			   for (int i = 1; i < subMatches; i++) {
			       ...
			       String n = prefix + matches + "." + s;
				   String v = rs.submatch(i)==null ? nullStr : rs.submatch(i);
			       props.put(n,v);
				   if (shouldTrack) {
						System.err.println("<extract>: " + n + "=" + v);
				   }
			   }
		       ...
		       for (int i = 0; i < sub.length; i++) {
				   ...
				   props.put(prefix + s, sub[i]);
				   if (shouldTrack) {
						System.err.println("<extract>: " + prefix + s + "=" +
								sub[i]);
				   }
				   matches++;
				   }
  ...
  public boolean isTransient() {
    ...
    static class ReStringTokenizer extends StringTokenizer {
