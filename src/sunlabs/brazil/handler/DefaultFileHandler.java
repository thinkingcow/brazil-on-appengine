...
public class DefaultFileHandler implements Handler {
	...
	if (!request.getUrl().startsWith("/") || !request.getUrl().endsWith("/")) {
	    return false;
	}
	String root = request.getProps().getProperty(prefix + ROOT,
		request.getProps().getProperty(ROOT, "."));
	String dir = root + FileHandler.urlToPath(request.getUrl());
...
*/
	...
	request.getProps().put("DirectoryName", base.getPath());

	StringTokenizer st = new StringTokenizer(request.getProps().getProperty(
	...
	while (st.hasMoreTokens()) {
	    ...
	    if (name.isFile()) {
	    	request.setUrl(request.getUrl() + token);
		request.getProps().put("fileName", name.getPath());
		request.log(Server.LOG_DIAGNOSTIC, prefix, 
