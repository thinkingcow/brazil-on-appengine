...
public class SimpleSessionHandler implements Handler {
  ...
  init(Server server, String prefix) {
    ...
    extract = server.getProps().getProperty(prefix + "extract", "${ipaddress}");
    force = (server.getProps().getProperty(prefix + "force") != null);
    valueTemplate = server.getProps().getProperty(prefix + "value", "&");
    if (server.getProps().getProperty(prefix + "digest") != null) {
      try {
    ...
    String match = server.getProps().getProperty(prefix + "re", ".+");
    if (match.startsWith("!")) {
  ...
  respond(Request request) throws IOException {
    ...
    String current = request.getProps().getProperty(session);
    ...
    Props props = new Props(request.getHeaders(), "query.",
        request.getQueryData(null), request.getProps());
    props.extra("ipaddress", "unknown");
    props.extra("url",request.getUrl());
    props.extra("query",request.getQuery());
    props.extra("method",request.getMethod());
    ...
    String result = Format.subst(request.getProps(), valueTemplate, true); // ${} only, not '\'
    if (invert) {
    ...
    request.getProps().put(session, id);
    request.log(Server.LOG_DIAGNOSTIC, isMine.prefix(),
