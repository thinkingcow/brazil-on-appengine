 ...
 * export version: 2.3 
 * Copyright (c) 1998-2008 Sun Microsystems, Inc.
 *
 ...
 * Last modified by suhler on 08/07/02 10:39:51
 *
 * Version Histories:
 *
 * 2.10 08/07/02-10:39:51 (suhler)
 *   add strict types
 *
 * 2.9 08/03/04-10:59:22 (suhler)
 *   CookieFilter.java
 *
 ...
 *		"allow" parameter).
 * <dt>strictTypes 
 *		<dd>If defined, only mime types specifically listed with
...
public class FileHandler implements Handler {
    ...
    @Override
    public boolean
    init(Server server, String prefixArg)
    {
	this.prefix = prefixArg;
	urlPrefix = server.getProps().getProperty(prefix + PREFIX, urlPrefix);
	return true;
    }
	...
	throws IOException
    {
	if (!request.getUrl().startsWith(urlPrefix)) {
	    return false;
	}
	if ((request.getProps().getProperty(prefix + GETONLY)!=null) &&
	...
	String url = request.getUrl().substring(urlPrefix.length());
	...
	if (file.isDirectory()) {
	    ...
	    if (request.getUrl().endsWith("/") == false) {
		request.redirect(request.getUrl() + "/", null);
		return true;
	    }
     ...
     * @param props		    where to look
     * @param pre		prefix in properties table 
     */
	...
	try {
	    ...
	    setModified(request.getProps(), file.lastModified());
