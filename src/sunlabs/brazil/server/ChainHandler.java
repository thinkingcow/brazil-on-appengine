 ...
 * Last modified by suhler on 09/02/12 10:37:13
 *
 * Version Histories:
 *
 * 2.6 09/02/12-10:37:13 (suhler)
 *   add Information messate when "report" is set
 *
    ...
    implements Handler
{
	...
	StringTokenizer nameTokens = new StringTokenizer(str);

	Vector<Handler> handlerVec = new Vector<Handler>();
    ...
    {
	String className = server.getProps().getProperty(name + ".class");
	if (className == null) {
	    className = name;
	} else {
	    prefix = null;
	...
	try {
	    ...
	    System.err.println("Instantiating: (" + className + ") " + h);
	...
	for (int i = 0; i < handlers.length; i++) {
	    ...
	    if (handlers[i].respond(request)) {
	        if (report != null) {
		    request.log(Server.LOG_INFORMATIONAL, prefix, names[i] +
			    " handled: " + request.getUrl());
	            request.getProps().put(report, names[i]);
		}
		return true;
	    }
	}
